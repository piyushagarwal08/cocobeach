...
if (!class_exists('ERE_Agent_Admin')) {
    ...
    class ERE_Agent_Admin
    {
        ...
        public function register_custom_column_titles($columns)
        {
            ...
            $columns['agencies'] =esc_html__('Agencies', 'essential-real-estate');
            $new_columns = array();
            foreach ( $columns as $key => $value ) {
                $new_columns[$key] = $value;
                if ( $key == 'cb' ) {
                    $new_columns['thumb'] = '<i class="dashicons dashicons-format-image"></i>';
                }
            }
            return $new_columns;
        }
        ...
        public function display_custom_column($column)
        {
            ...
            switch ($column) {
                ...
                case 'mobile':
                    ...
                    break;
                case 'agencies':
                    $cate = ere_admin_taxonomy_terms($post->ID, 'agencies', 'agent');
                    $allowed_html = array(
                        'a' => array(
                            'href' => array(),
                            'title' => array(),
         ...
         * @return string
         */
        public function modify_agencies_slug($existing_slug)
        {
            $agencies_url_slug = ere_get_option('agencies_url_slug');
            if ($agencies_url_slug) {
                return $agencies_url_slug;
            }
            return $existing_slug;
        }
        /**
