	...
	class ERE_Shortcode_Property
	{
		...
		public function my_properties($atts)
		{
			...
			if (!empty($_REQUEST['property_id'])) {
				$property_identity = sanitize_text_field($_REQUEST['property_id']);
				$meta_query[] = array(
			...
			$my_properties_columns = apply_filters('ere_my_properties_columns', array(
				'picture' => esc_html__('Picture', 'essential-real-estate'),
				'title' => esc_html__('Title', 'essential-real-estate'),
				'id' => esc_html__('ID Number', 'essential-real-estate'),
			...
			ere_get_template('property/my-properties.php', array('properties' => $properties->query($args), 'max_num_pages' => $properties->max_num_pages, 'my_properties_columns' => $my_properties_columns, 'post_status' => $post_status, 'title' => $title, 'property_id' => $property_identity,'property_status'=>$property_status));
			wp_reset_postdata();
			return ob_get_clean();
		}
		...
		public function my_save_search($atts)
		{
			...
			$my_save_search_columns = apply_filters('ere_my_save_search_columns', array(
				'title' => esc_html__('Saved Searches', 'essential-real-estate')
			));
			echo $this->ere_message;
			ere_get_template('property/my-save-search.php', array('save_seach' => $results,'my_save_search_columns' => $my_save_search_columns));
			return ob_get_clean();
		}
		/**
