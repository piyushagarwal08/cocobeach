...
if (!class_exists('GSF_Field_Repeater')) {
	class GSF_Field_Repeater extends GSF_Field
	{
		function html_start()
		{
			...
			?>
			<div <?php echo (!empty($field_id) ? 'id="' . esc_attr($field_id) . '"' : ''); ?> class="<?php echo esc_attr($wrapper_class); ?>" <?php $this->the_required(); ?>>
				<?php $this->html_label(); ?>
			<?php
		}
		function html_end() {
			...
			<div class="gsf-row gsf-repeater-footer">
				<?php foreach ($this->params['fields'] as $field): ?>
					...
					<div class="gsf-col gsf-col-<?php echo esc_attr($col); ?>">
						<p class="gsf-desc"><?php echo esc_attr(isset($field['desc']) ? $field['desc'] : ''); ?></p>
					</div>
				<?php endforeach;?>
			</div><!-- /.gsf-row -->
			<?php
		...
		function html_repeater_label() {
			...
			<div class="gsf-row gsf-repeater-header">
				<?php foreach ($this->params['fields'] as $field): ?>
					...
					?>
					<div class="gsf-col gsf-col-<?php echo esc_attr($col); ?>">
						<label class="gsf-label"><?php echo esc_attr(isset($field['title']) ? $field['title'] : ''); ?></label>
					</div>
				<?php endforeach;?>
			</div><!-- /.gsf-row -->
			<?php
