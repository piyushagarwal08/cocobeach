<?php
$agencies = $layout_style = $item_amount = $items = $image_size = $show_paging = $dots = $nav = $nav_position = $autoplay = $autoplaytimeout =
			...
			<div class="<?php echo join(' ', $wrapper_classes) ?>" <?php echo implode(' ', $wrapper_attributes); ?>>
				...
				while ($data->have_posts()): $data->the_post();
					...
					if (preg_match('/\d+x\d+/', $image_size)) {
						...
						$avatar_src = ere_image_resize_id($avatar_id, $image_sizes[0], $image_sizes[1], true);
						if (empty($avatar_src)) {
					...
					$padding_bottom = '';
					if(!empty( $default_avatar_src )) {
						if(empty( $width ) || empty( $height )) {
							$padding_bottom = 'padding-bottom: 125.93%';
						} else {
							$padding_bottom = 'padding-bottom: '.round(($height/$width)*100, 2).'%';
						}
					}
					...
					<div class="agent-item <?php echo esc_attr($gf_item_wrap) ?>">
						<div class="agent-item-inner">
							...
							<?php else: ?>
								<div class="agent-avatar agent-avatar-bg" style="background-image: url(<?php echo esc_url($default_avatar_src) ?>);<?php echo esc_attr( $padding_bottom ) ?>"></div>
							<?php endif; ?>
							...
							<div class="agent-info">
								...
								<?php if (!empty($agent_description) && ($layout_style == 'agent-list')): ?>
									<p><?php echo esc_attr($agent_description) ?></p>
								<?php endif; ?>
							</div>
